...
; put over the following line by EV
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Style/Chrome Image..."
...
(define (script-fu-cross-light
  ...
  (let* (
    ...
    (set! currentselection (car(gimp-selection-save img))) ; these two lines are new in the new version 
...
; this line was moved down by EV
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Style/Cross Light..."
  "Cross light effect"
  "Iccii <iccii@hotmail.com>"
  "Iccii"
  "2001, Aug"
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Artist/Note Paper..."
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Artist/Pastel..."
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Texture/Patchwork..."
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Texture/Stained Glass..."
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Texture/Texturizer..."
...
(define (script-fu-water-paint-effect-G2
	...
	(gimp-undo-push-group-start inImage)
        (set! currentselection (car(gimp-selection-save inImage))) ; these two lines are new in the new version 
...
;	(gimp-image-add-layer inImage tmp-layer -1)
    ...
    (gimp-selection-load currentselection) ; these seven lines are new in the new version
...
(script-fu-register
	...
	"<Image>/Script-Fu/Photo Effects/Artist/Water Paint Effect..."
...
(define (script-fu-wrap-effect-G2
	...
	(set! theOld-bg (car (gimp-palette-get-background)))
	(set! theNewlayer (car (gimp-layer-copy inDrawable 1)))
...
(script-fu-register
	...
	"<Image>/Script-Fu/Photo Effects/Artist/Wrap Effect..."
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Artist/Stamp..."
...
(define (script-fu-brick-wall-G2
  ...
  (let* (
	...
	;; plug-in-pxelize を tmp-layer1 に適用するため、はみ出ている部分を引き伸ばして着色
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Texture/Brick Wall..."
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/The Others/High Pass..."
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Style/Soft Focus..."
  "Soft focus effect"
  "Iccii <iccii@hotmail.com>"
  "Iccii"
  "2001, Jul"
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Style/Solarization..."
...
(script-fu-register
  "script-fu-scroll-G2"
  "<Image>/Script-Fu/Photo Effects/The Others/Scroll..."
...
;*************************************************************************************** 
; Draw dotted image script  for GIMP 1.2
; Copyright (C) 2001 Iccii <iccii@hotmail.com>
; 
; --------------------------------------------------------------------
; version 0.1  by Iccii 2001/07/17
;     - Initial relase
; version 0.1a by Iccii 2001/07/18
;     - Add BG color/layer-opacity option
; version 0.1b by Iccii 2001/07/24
;     - Remove BG color/layer-opacity option
;     - Optimize (speed up) ...thanks to Sakai
; version 0.1c by Iccii 2001/08/21
;     - Added this line: (define (list-ref l n) (nth n l))
; version 0.2  by Iccii 2002/05/20
;     - Re-written all code (totally new algorithm)
;     - Enable brush stroke, angle, coloring, etc
;
; --------------------------------------------------------------------
; 
; This program is free software; you can redistribute it and/or modify
...
;
	...
	;; 点描スクリプト
(define (script-fu-paint-dot-image-G2
			img		;; 対象画像
			layer		;; 対象レイヤー
			tool-type	;; 描画ツールの種類
			density		;; 描画の繰り返し回数
			brush		;; 描画に使用するブラシの種類
			lengths		;; ブラシストロークの長さ
			length-rand	;; ブラシストロークを変化させる
			angle		;; ブラシストロークの方向
			angle-rand	;; ブラシストロークの方向を変化させる
			color-rand	;; 色を変化させる
			undo?		;; アンドゥを有効にするかどうか
	)
  ...
  (define dot-proc
    (cond
      ((= tool-type 0) gimp-pencil)
      ((= tool-type 1) gimp-airbrush-default)
      ((= tool-type 2) gimp-paintbrush-default)
  ))

	;; min-val から max-val までのランダムな数を返す
  (define (get-random-number min-val max-val)
    (+ min-val (rand (- max-val min-val))))

	;; always 0 <= number <= 255
  (define (color-round number)
    (min (max number 0) 255))
  ...
  (define (cos-s ang)
	...
	;; [0 to 127, 128 to 255] ... noraml color range
	;; [0 to 127,-128 to  -1] ... color range which gimp-drawable-get-pixel returned

  (define (convert-numerical-region region color)
    (if (= region 0)
	;; input :        -128 <= color <= 127
	;; return: always    0 <= color <= 255
        (let ((N (min (max color -128) 127)))
          (if (< N 0) (+ N 256) N))
	;; input :           0 <= color <= 255
	;; return: always -128 <= color <= 127
        (let ((N (min (max color    0) 255)))
          (if (> N 127) (- N 256) N))))
  ...
  (let* (
         ...
         (dot-layer (car (gimp-layer-new img width height 1
                                         "Dot image" 100 NORMAL)))
         (x-coordinate '())
         (y-coordinate '())
         (color-list '())
         (count 0)
         (dummy (gimp-brushes-set-brush (car brush)))
         (brush-info (gimp-brushes-get-brush))
         (brush-area (* (+ lengths (* length-rand 0.5))
                        (cadr brush-info) (caddr brush-info)))
         (layer-area (* width height))
         (iteration (* 0.125 density (/ layer-area brush-area)))
        )
...
; the next line was moved one up by EV
    (gimp-image-add-layer img dot-layer -1)
    (gimp-edit-clear dot-layer)

	;; 描画座標と描画色を求める
    (while (> iteration count)
      (let* ((x1 (get-random-number 0 (- width 1)))
             (y1 (get-random-number 0 (- height 1)))
             (angle-rand-angle (get-random-number (- angle-rand) angle-rand))
             (length-rand-length (get-random-number 0 length-rand))
             (x2 (+ x1 (* (+ lengths length-rand-length)
                          (cos-s (+ angle angle-rand-angle)))))
             (y2 (+ y1 (* (+ lengths length-rand-length)
                          (sin-s (+ angle angle-rand-angle)))))
             (pixel (cadr (gimp-drawable-get-pixel layer x1 y1)))
             (R (color-round (+ (convert-numerical-region 0 (aref pixel 0))
                                (get-random-number (- color-rand) color-rand))))
             (G (color-round (+ (convert-numerical-region 0 (aref pixel 1))
                                (get-random-number (- color-rand) color-rand))))
             (B (color-round (+ (convert-numerical-region 0 (aref pixel 2))
                                (get-random-number (- color-rand) color-rand))))
            )
        (set! x-coordinate (cons x1 x-coordinate))
        (set! y-coordinate (cons y1 y-coordinate))
        (set! x-coordinate (cons x2 x-coordinate))
        (set! y-coordinate (cons y2 y-coordinate))
        (set! color-list (cons (list R G B) color-list))
        (set! count (+ count 1))
    )) ; end of while
    (set! count 0)
    (set! length-max (length color-list))

	;; 本体 (描画処理)
    (while (> length-max count)
      (gimp-palette-set-foreground (list-ref color-list count))
      (aset strokes 0 (list-ref x-coordinate (* 2 count)))
      (aset strokes 1 (list-ref y-coordinate (* 2 count)))
      (aset strokes 2 (list-ref x-coordinate (+ 1 (* 2 count))))
      (aset strokes 3 (list-ref y-coordinate (+ 1 (* 2 count))))
      (dot-proc dot-layer 4 strokes)
      (set! count (+ count 1))
    ) ; end of while
...
(script-fu-register
  "script-fu-paint-dot-image-G2"
  "<Image>/Script-Fu/Photo Effects/Artist/Paint Dot Image..."
  "Paint dot image"
  "Iccii <iccii@hotmail.com>"
  "Iccii"
  "2002, May"
  ...
  SF-IMAGE      "Image"            0
  SF-DRAWABLE   "Drawable"         0
  SF-OPTION     "Paint Tool"       '("Pencil" "Airbrush" "Paintbrush")
  SF-ADJUSTMENT "Density (%)"      '(50 1 200 1 10 0 0)
  SF-BRUSH      "Use Brush"        '("Circle Fuzzy (07)" 1.0 20 0)
  SF-ADJUSTMENT "Brush Length"     '(9 0 30 1 1 0 0)
  SF-ADJUSTMENT "Length Randomize" '(5 0 20 1 1 0 0)
  SF-ADJUSTMENT "Brush Angle"      '(45 0 360 1 15 0 0)
  SF-ADJUSTMENT "Angle Randomize"  '(30 0 180 1 15 0 0)
  SF-ADJUSTMENT "Color Randomize"  '(10 0 127 1 16 0 0)
  SF-TOGGLE     "Enable Undo"      FALSE
)
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Style/Funky Color..."
...
(script-fu-register
  "script-fu-cutout-G2"
  "<Image>/Script-Fu/Photo Effects/Artist/Cutout..."
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Artist/Color Pencil..."
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Artist/Palette Knife..."
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Brush/Angled strokes..."
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Brush/Crosshatched..."
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Artist/Drawing..."
  "Creates a drawing."
  "Eddy Verlinden <eddy_verlinden@hotmail.com>"
  "Eddy Verlinden"
  "2007, juli"
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Artist/Cartoon..."
...
(script-fu-register
  ...
  "<Image>/Script-Fu/Photo Effects/Artist/Cartoon2..."
  "Creates a drawing effect"
  "Eddy Verlinden <eddy_verlinden@hotmail.com>"
  "Eddy Verlinden"
  "2007, juli"
...
(script-fu-register
  "script-fu-inkpen-G2"
  "<Image>/Script-Fu/Photo Effects/Artist/Inkpen..."
...
(script-fu-register
  "script-fu-conte-G2"
  "<Image>/Script-Fu/Photo Effects/Artist/Conte..."
